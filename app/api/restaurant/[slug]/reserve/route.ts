import { findAvailabileTables } from "@/services/restaurannt/findAvailableTables";
...
export async function GET(req: NextRequest, res:NextResponse) {
  const { slug, day, time, partySize } = getQueryParams(req.url);
...
if (!searchTimesWithTables) {
  return NextResponse.json(
    {
      errorMessage: "Invalida data provided",
    },
    { status: 400 }
  );
}
